  ...
  <div class="w-full px-2 pt-2">
    <div v-if="wishCnt === 1">
      ...
      <button @click="confirmWish()"
        class="interRed mt-2 text-yellow w-full mx-auto block border-2 border-dotted rounded-lg py-1 hover:border-solid">实现并折叠愿望</button>
    </div>
    <div v-if="wishCnt != 1">
      <h2 class="text-center mb-2">这个愿望已被实现或正在入队❌</h2>
    </div>
    <button @click="backToHomeView()"
      class="interRed mt-2 text-yellow w-full mx-auto block border-2 border-dotted rounded-lg py-1 hover:border-solid">返回愿望队列</button>
  </div>
</template>
